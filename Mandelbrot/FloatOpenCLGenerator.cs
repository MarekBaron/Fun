...
{
    class FloatOpenCLGenerator : IGenerator
    {
        ...
        public byte[] Generate(int aWidth, int aHeight, double aSetStartX, double aSetWidth, double aSetStartY, double aSetHeight, byte[] aPalette)
        {
            ...
            _kernel.SetMemoryArgument(0, ValueToComputeBuffer((float)aSetStartX));
            _kernel.SetMemoryArgument(1, ValueToComputeBuffer((float)aSetWidth / aWidth));
            _kernel.SetMemoryArgument(2, ValueToComputeBuffer((float)aSetStartY));
            _kernel.SetMemoryArgument(3, ValueToComputeBuffer((float)(aSetHeight / aHeight)));
